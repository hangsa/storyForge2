...
2. The wizard's `finishWizard` currently navigates to `/stage1` (per Task 2c34018's earlier fix). Post-workspace, the natural landing is `/workspace?mode=managed`. This is flagged under "Open questions" but **not** a task — it is a behavior change the user should approve before plan execution begins.

---

## Open Questions (for the user before execution begins)

1. **Wizard finish navigation.** After this plan lands, the wizard's `finishWizard` still navigates to `/stage1`. The spec implicitly wants `/workspace?mode=managed` (workspace is the post-init hub). Should Tasks 1–17 be extended with one more task to flip the wizard finish too? Or keep `/stage1` as the post-wizard landing?
2. **`ContextPanel` placeholder depth.** Task 13 ships tabs that resolve to API fetches + a thin placeholder body. The alternative is to refactor Stage1Page/Stage2Page/Stage3Page to expose their inner tabs and mount them inside ContextPanel. This is more work but delivers real inline editing in v1.8. The spec's `Non-Goals` line says the latter is out of scope. Confirm the placeholder is acceptable.
3. **`Stage5`/`Stage6` modes.** Stage5/6 currently have no inline-edit affordance; clicking the sidebar opens them as separate routes. Task 1 redirects those routes to `/workspace?panel=diagnosis|export`. Inside ContextPanel (Task 13), the diagnosis/export tabs are placeholder. Confirm this fallback is acceptable.

If "yes" to all three, the plan is ready to execute. If "no" to any, the plan needs an extra task before Task 1 begins.
