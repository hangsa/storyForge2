  ...
  return render(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        ...
        <Route path="/redirect-stage4" element={<NavigateTo to="/workspace?mode=manual" />} />
        <Route path="/redirect-stage5" element={<NavigateTo to="/workspace?mode=manual&panel=diagnosis" />} />
        <Route path="/redirect-stage6" element={<NavigateTo to="/workspace?mode=manual&panel=export" />} />
      </Routes>
    </MemoryRouter>,
  );
}
function NavigateTo({ to }: { to: string }) {
  // Simulate a <Navigate replace to=...> by re-rendering with the new path
