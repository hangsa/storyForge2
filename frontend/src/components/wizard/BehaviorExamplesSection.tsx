  ...
  return (
    <div className="space-y-4" data-testid="behavior-examples-section">
      <div className="flex items-center justify-between">
        <h3 className="font-label-mono text-system-log text-[10px] uppercase tracking-wider">
          行为示例
        </h3>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={add}
            className="rounded-full border border-dashed border-outline-variant px-3 py-1.5 font-body-ui text-xs text-system-log/70 transition-colors hover:border-primary-container/50 hover:text-primary-container"
          >
            + 添加示例
          </button>
        </div>
        ...
        {examples.map((ex, idx) => (
          ...
          >
            <div className="mb-1 flex items-center justify-between">
              <span className="font-label-mono text-system-log/80 text-[10px]">
                示例 #{idx + 1}
              </span>
              <button
                type="button"
                ...
                onClick={() => remove(idx)}
                className="p-1 font-body-ui text-xs text-system-log/70 hover:text-error"
                aria-label="删除示例"
              >
                删除
              </button>
            ...
            <label className="block">
              <span className="mb-1 block font-label-mono text-system-log/80 text-[10px]">
                情境
              </span>
              <AutoTextarea
                value={ex.situation}
