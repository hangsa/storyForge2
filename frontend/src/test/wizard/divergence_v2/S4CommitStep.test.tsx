...
describe("S4CommitStep", () => {
  it("shows empty state when committedConcept is null", () => {
    render(<S4CommitStep committedConcept={null} noveltyScores={null} loading={false} onEditConcept={vi.fn()} onRegenerateCommit={vi.fn()} onRegenerateAllDivergence={vi.fn()} onAdvance={vi.fn()} />);
    expect(screen.getByText(/尚未合成 concept/)).toBeInTheDocument();
  });

  it("renders all 5 fields and novelty grade", () => {
    render(<S4CommitStep committedConcept={MOCK_CONCEPT} noveltyScores={MOCK_SCORES} loading={false} onEditConcept={vi.fn()} onRegenerateCommit={vi.fn()} onRegenerateAllDivergence={vi.fn()} onAdvance={vi.fn()} />);
  ...
  it("edit → save calls onEditConcept", () => {
    const onEdit = vi.fn();
    render(<S4CommitStep committedConcept={MOCK_CONCEPT} noveltyScores={null} loading={false} onEditConcept={onEdit} onRegenerateCommit={vi.fn()} onRegenerateAllDivergence={vi.fn()} onAdvance={vi.fn()} />);
  ...
  it("'重新生成' triggers confirm dialog", () => {
    const onRegen = vi.fn();
    render(<S4CommitStep committedConcept={MOCK_CONCEPT} noveltyScores={null} loading={false} onEditConcept={vi.fn()} onRegenerateCommit={onRegen} onRegenerateAllDivergence={vi.fn()} onAdvance={vi.fn()} />);
  ...
  it("'下一步' calls onAdvance", () => {
    ...
    render(<S4CommitStep committedConcept={MOCK_CONCEPT} noveltyScores={null} loading={false} onEditConcept={vi.fn()} onRegenerateCommit={vi.fn()} onRegenerateAllDivergence={vi.fn()} onAdvance={onAdvance} />);
    fireEvent.click(screen.getByRole("button", { name: /下一步/ }));
    expect(onAdvance).toHaveBeenCalled();
  });

  it("edited_by_user=true shows '用户已编辑' label", () => {
    render(<S4CommitStep committedConcept={{ ...MOCK_CONCEPT, edited_by_user: true }} noveltyScores={null} loading={false} onEditConcept={vi.fn()} onRegenerateCommit={vi.fn()} onRegenerateAllDivergence={vi.fn()} onAdvance={vi.fn()} />);
