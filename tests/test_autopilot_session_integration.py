...
def test_full_chapter_pipeline_via_runner_loop(client, projects_dir):
    ...
    # 4. Tasks ran in priority order; executor saw each exactly once
    assert completed_ids == ["plan", "write", "review", "archive"]
    assert [c["item_id"] for c in executor.calls] == ["plan", "write", "review", "archive"]
    ...
    #    (per add_queue) + 4 task_complete + 4 queue_drop (per runner step).
    types = [e.type for e in s.history]
    ...
    assert set(add_indices) == set(drop_indices) == {
        "plan", "write", "review", "archive",
    }
    ...
    drop_order = [e.task_id for e in s.history if e.type == "queue_drop"]
    assert drop_order == ["plan", "write", "review", "archive"]
